...
export function useReports(filter: ReportsFilter) {
  ...
  return useQuery({
    queryKey: ['reports', userId, filter],
    queryFn: async () => {
      ...
      // 2. Fetch reports data client-side
      let reportData;
      let prevReportData = null;
      let monthCompareData = null;

      if (isMonthComparison) {
        const resolvedMonthStr = filter.monthStr || currentMonthStr;
        ...
        try {
          ...
          console.warn('Failed to resolve previous month comparison:', err);
        }
      } else {
        ...
        try {
          ...
          console.warn('Failed to resolve previous range comparison:', err);
        }
      }

      // 3. Resolve Budget Context details
