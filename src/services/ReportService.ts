...
export class ReportService {
  ...
  public getCachedMonthlyReport(userId: string, monthStr: string): MonthlyReport | null {
    const cacheKey = `${userId}-monthly-${monthStr}`;
  ...
  async getMonthlyReport(userId: string, monthStr: string): Promise<MonthlyReport> {
    ...
    if (this.cache.has(cacheKey)) {
      logger.debug(`ReportService: Resolving monthly report for ${monthStr} for user ${userId} from cache.`);
    ...
    const rows = await fetchExpensesWithCategories(userId, startDate, endDate);
  ...
  async getYearlyReport(userId: string, year: number): Promise<YearlyReport> {
    const cacheKey = `${userId}-yearly-${year}`;
    if (this.cache.has(cacheKey)) {
      logger.debug(`ReportService: Resolving yearly report for ${year} for user ${userId} from cache.`);
      return this.cache.get(cacheKey);
    }
  ...
  async getCustomReport(
    ...
    const cacheKey = `${userId}-custom-${startDate.getTime()}-${endDate.getTime()}`;
    if (this.cache.has(cacheKey)) {
      logger.debug(`ReportService: Resolving custom report for user ${userId} from cache.`);
      return this.cache.get(cacheKey);
    }
  ...
  async getPresetReport(userId: string, preset: DateRangePreset): Promise<ReportData> {
    const cacheKey = `${userId}-preset-${preset}`;
    if (this.cache.has(cacheKey)) {
      logger.debug(`ReportService: Resolving preset report for ${preset} for user ${userId} from cache.`);
      return this.cache.get(cacheKey);
    }
  ...
  async compareMonths(
    ...
    const [current, previous] = await Promise.all([
      this.getMonthlyReport(userId, currentMonthStr),
      this.getMonthlyReport(userId, previousMonthStr),
    ]);
