...
export class DashboardService {
  ...
  public async getDashboardData(userId: string, monthStr: string): Promise<DashboardData> {
    if (!userId) {
      throw new Error('User ID is required.');
    }
    const sTime = Date.now();
    logger.debug(`DashboardService: Compiling dashboard for month ${monthStr} for user ${userId}`);

    const cachedReport = this.reportService.getCachedMonthlyReport(userId, monthStr);
    ...
    if (cachedReport) {
      ...
      const [fetchedBudget, fetchedRecent, monthlyExpenses] = await Promise.all([
        ...
        this.expenseRepo.getExpenses(userId, { limit: 5 }),
    ...
    logger.debug(`[PERF LOG] Dashboard Service batch loaded in ${Date.now() - sTime}ms.`);
